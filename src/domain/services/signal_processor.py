...
class BandpassFilter:
    ...
    """
    def __init__(self, fs: float, lowcut: float = 0.5, highcut: float = 40.0, order: int = 4) -> None:
        if fs <= 0:
...
class RPeakDetector:
    ...
    def find_r_peaks(self, signal: npt.NDArray[np.float64]) -> npt.NDArray[np.int64]:
        if signal.size < int(self.fs):
            raise InvalidInputError("Signal is too short (less than 1 second) for reliable detection.")
